...
public class ProcessUtils {
    ...
    public final static Logger LOG = LoggerFactory.getLogger(ProcessUtils.class);
    ...
    public static int select(boolean v, int telnetPortPid) throws InputMismatchException {
        ...
        if (processMap.isEmpty()) {
            LOG.info("Can not find java process. Try to pass <pid> in command line.");
            return -1;
        }

        LOG.info("Found existing java process, please choose one and hit RETURN.");
        // print list
        int count = 1;
    ...
    public static Map<Integer, String> listProcessByJps(boolean v) {
        ...
        LOG.debug("Try use jps to lis java process, jps: " + jps);
    ...
    private static File findJps() {
        ...
        for (String path : paths) {
            ...
            if (jpsFile.exists()) {
                LOG.debug("Found jps: " + jpsFile.getAbsolutePath());
                jpsList.add(jpsFile);
            }
        }

        if (jpsList.isEmpty()) {
            LOG.debug("Can not find jps under :" + javaHome);
            String javaHomeEnv = System.getenv("JAVA_HOME");
            LOG.debug("Try to find jps under env JAVA_HOME :" + javaHomeEnv);
            for (String path : paths) {
        ...
        if (jpsList.isEmpty()) {
            LOG.debug("Can not find jps under current java home: " + javaHome);
            return null;
        }
